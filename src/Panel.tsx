...
function Dashboard() {
  ...
  useEffect(() => { if (!routeEventId || !attendeeQuery.data) return; setEventName(attendeeQuery.data.event.name); setEventLink(attendeeQuery.data.event.eventLink || ''); setEventContext(attendeeQuery.data.event.context || ''); setGuestText(attendeeQuery.data.attendees.map((attendee) => attendee.originalName).join('\n')) }, [routeEventId, attendeeQuery.data])
  const importer = useMutation({ mutationFn: async () => importEvent(await getToken(), eventName, attendees, eventContext, eventLink), onSuccess: (result) => { setEventId(result.eventId); navigate(`/app/events/${result.eventId}`); setSelectedId(null); setError('') }, onError: (caught) => setError(caught.message) })
