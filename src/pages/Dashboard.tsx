...
export default function Dashboard() {
  ...
  return <main className="app-shell gl-dashboard">
    ...
    <div className="gl-dashboard-grid">
      <section className="gl-dashboard-card gl-event-history"><div className="gl-panel-header"><div><div className="gl-panel-title">Event history</div><div className="gl-dashboard-subtitle">Your recent lookups</div></div><button className="gl-btn-ghost" onClick={() => navigate('/app/events/new')}>New event</button></div><div className="gl-history-list">{events.isLoading ? <div className="gl-dashboard-empty">Loading events…</div> : !events.data?.events.length ? <div className="gl-dashboard-empty">No events yet. Start by building your first network.</div> : events.data.events.map((event) => <div className="gl-history-row" key={event.id}><button onClick={() => navigate(`/app/events/${event.id}`)}><strong>{event.name}</strong><span>{event.context || 'Canada'} · {event.people} people · {event.resolved} resolved</span></button><div className="gl-history-actions">{event.eventLink && <a href={event.eventLink} target="_blank" rel="noreferrer">↗</a>}<button onClick={() => navigate(`/app/events/${event.id}`)}>Open →</button></div></div>)}</div></section>
